...
ocr = PaddleOCR(
    ...
    #cls_model_dir="C:/paddle_model/ch_ppocr_mobile_v2.0_cls_infer",
    use_doc_orientation_classify=True,
    ...
    if matches_date:
        try:
            if len(matches_date) > 1:
                ...
                first_date_index = text.find(matches_date[1])
            else:
                dob_paddle = re.sub(r'[\s.\n-]+', '', matches_date[0])
                first_date_index = text.find(matches_date[0])

            date_obj = datetime.strptime(dob_paddle, "%d%b%Y")
            # Convert to YYYY-MM-DD
            dob_paddle = date_obj.strftime("%Y-%m-%d")
            # Remove all text before the selected date and dates from text
            text_without_dates = pattern_date.sub('', text[first_date_index:])
